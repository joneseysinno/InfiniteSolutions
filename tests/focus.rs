//! E19 — session focus, typed payload, undo restores the commit.
...
#[test]
fn click_focuses_an_accepting_field_and_typing_commits() {
    let (_dir, store) = seeded();
    let field = addresses::innovator_field_a_key();
    let before = field_text(&store, field);
    assert_eq!(before, "height");

    click(&store, field);
    assert_eq!(
        store.stored_at(addresses::focus_key()).as_deref(),
        Some(field),
        "clicking an accepts text space writes session focus"
    );

    type_key(&store, "KeyX");
    assert_eq!(
        field_text(&store, field),
        "heightx",
        "characters amend the focused field's payload through the composition"
    );
    ...
    assert_eq!(
        field_text(&store, field),
        before,
        "Ctrl+Z / undo restores the previous payload (D49)"
    );
}
