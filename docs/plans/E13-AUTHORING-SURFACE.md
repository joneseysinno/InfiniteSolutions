...
> **Status:** draft 1, 2026-08-28. **E13.0–E13.4 landed 2026-08-28.** R20: every **Verified by**
...
| **E13.3** | The property inspector writes | landed | `tests/inspector_write.rs` | Edit a field; the record changes; the canvas follows in the same frame; `Ctrl+Z` puts it back (E12). The write goes through the interpreted behaviour composition, not through `store.amend` — E6's discipline, applied to the second input surface |
