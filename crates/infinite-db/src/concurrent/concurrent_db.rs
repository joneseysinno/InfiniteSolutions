...
impl InfiniteDb {
    ...
    /// Like [`Self::insert`], but returns [`EngineError::QueueFull`] instead of waiting.
    pub fn try_insert(
        &self,
        space: SpaceId,
        point: DimensionVector,
        ...
        let config = self
            ...
            .cloned()
            .ok_or(EngineError::SpaceNotFound(space))?;
        crate::engine::write_validate::validate_point_write(&config, &point, false)?;
        let rev = self.next_revision();
        let address = Address::new(space, point.clone());
        let key = space_key(&self.spaces.read(), space, &point);
        let hilbert_key = HilbertKey(key);
        ...
        match self.coordinator.try_enqueue_write(job) {
            Ok(true) => Ok(rev),
            Ok(false) => {
                self.session_watermarks
                    .retire_failed(rev, "write queue full".to_string());
    ...
    /// When `pause` is true, shard I/O threads stop draining. The write queues can
    /// then fill so [`Self::try_insert`] returns [`EngineError::QueueFull`] (D33).
    pub fn pause_write_drain(&self, pause: bool) {
        self.coordinator.pause_drain(pause);
    }
