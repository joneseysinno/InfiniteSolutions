...
impl WriteQueueSender {
    ...
    /// Offer a write without waiting. `Ok(false)` means the queue is full.
    ///
    /// D24 / D33: the facade's `StoreWrite::submit` must return `Full` rather than
    /// stalling a tick. `enqueue_write` uses `send` and blocks; this is the
