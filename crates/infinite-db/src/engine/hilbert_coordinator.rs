...
impl HilbertCoordinator {
    ...
    /// Offer a write without waiting. `Ok(false)` means the target shard's queue is full.
    pub fn try_enqueue_write(&self, job: WriteJob) -> io::Result<bool> {
        debug_assert_eq!(job.branch_id, BranchId::MAIN);
        let space = job.space_id();
        let shard_bits = self.shard_bits_for_space(space);
        let shard_id = if job.hilbert_key != HilbertKey::ZERO {
        ...
        let key = ShardKey::new(space, shard_id);
        let shard = self.shards.get(&key).expect("shard just ensured");
        shard.queue.try_enqueue_write(job)
    }

    /// Stop or resume shard I/O `recv` so a caller can fill the bounded queue.
    pub fn pause_drain(&self, pause: bool) {
        self.config
            .drain_paused
            .store(pause, std::sync::atomic::Ordering::SeqCst);
    }
